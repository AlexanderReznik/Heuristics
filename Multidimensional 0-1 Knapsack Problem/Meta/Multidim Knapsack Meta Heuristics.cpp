...
void findTabuImprovement2(int n, int m, int* x, int** r, int* used, int* b, int* p, int* tabu, int iteration, int& currentCost, int& bestImprovementCost, int& swap1, int& swap2)
{
	for (int c1 = 0; c1 < n; c1++)
	{
		for (int c2 = c1 + 1; c2 < n; c2++)
...
int getTabuTenure(int n)
{
	return round(sqrt(n)/2);
}

void makeTabuMove2(int n, int m, int* x, int** r, int* used, int* tabu, int iteration, int& currentCost, int& bestImprovementCost, int& swap1, int& swap2)
{
	if (bestImprovementCost > INT_MIN)
	{
		for (int i = 0; i < m; i++)
		{
			int constraintDif = (!x[swap1] - x[swap1]) * r[i][swap1] + (!x[swap2] - x[swap2]) * r[i][swap2];
		...
		tabu[swap1] = iteration + getTabuTenure(n);
		tabu[swap2] = iteration + getTabuTenure(n);

		currentCost = bestImprovementCost;
	}
}

void tabuSearch(int n, int m, int* x, int* b, int* p, int** r)
{
	...
	int bestCost = getCost(n, currentSolution, p);
	...
	while (iteration < 10000000000/n/n/sqrt(n))
	{
		iteration++;
		bestImprovementCost = INT_MIN;
		findTabuImprovement2(n, m, currentSolution, r, used, b, p, tabu, iteration, currentCost, bestImprovementCost, swap1, swap2);
		makeTabuMove2(n, m, currentSolution, r, used, tabu, iteration, currentCost, bestImprovementCost, swap1, swap2);
...
int main(int argc, char* argv[])
{
	...
	//tabuSearch(n, m, x, b, p, r);
	double duration = solveBestImprovementWithTime(n, m, x, b, p, r, metaheuristic);

	//printSolution(n, x);
	//std::cout << isFeasible(n, m, x, r, b) << ' ';
	std::cout << getCost(n, x, p) << ' ' << duration*1.0/1000000 << '\n';
